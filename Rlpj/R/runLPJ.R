...
#'
#' # settings
#' settings <- list(gridList = gridList, scale = "global",mode = "cf",
...
#'        Starting run
#'        Calling LPJ-GUESS#'
#'        "/some/absolute/path/guess -input cf "/some/absolute/path/runDirectory/global_cf.ins
#'
#'        Finished run
#'
#'  result
...
#'      grid cells         : gridlist_geb.txt
#'      LPJ model outputs  : 39 outputs
#'      aaet  agpp	aiso	amon	anpp	cflux	clitter	cmass	cpool	cton_leaf	dens
#'      firert	fpc	speciesheight	lai	maet	mevap	mgpp	mintercep	miso	mlai
#'      mmon	mnee	mnpp	 pet	mra	mrh	mrunoff	mwcont_lower	mwcont_upper
#'      nflux	ngases	nlitter	nmass	npool	nsources	nuptake	runoff	vmaxnlim
#'
#' #Parallel Run
#'
...
setGeneric("runLPJ",
           function(x=NULL, typeList=NULL, parameterList=NULL, settings = NULL)
             standardGeneric("runLPJ"))


setMethod('runLPJ', signature(x='character'),
          function(x=NULL, typeList=NULL, parameterList=NULL, settings = NULL){
            ...
            singleRun <- createSingleObject(x, typeList, settings)
...
setMethod('runLPJ', signature(x='LPJSetup'),
          function(x=NULL, typeList=NULL, parameterList=NULL, settings=NULL){
            if (is.null(settings) || !typeof(settings) == "list"){
              stop("invalid settings provided")
            }
            if(!file.exists(x[["mainDir"]])){
              stop("Invalid main directory")
            }
            if (is.null(parameterList)){
            ...
            for (i in 1:length(parameterList)){
              ...
              singleRun$template1Mem <- sub("path_to_output/",
                                            paste(singleRun$outDir, "/", sep =""), singleRun$template1Mem)
              for ( j in 1:length(singleRun$typeList)) {
                singleRun$template1 <- sub(paste("! file", singleRun$typeList[j], sep="_"),
                                           paste("file",  singleRun$typeList[j], sep="_") , singleRun$template1)
              }
              # template 2: the cru or cf template
              singleRun$template2Mem<- readLines(file.path(singleRun$mainDir,singleRun$template2))
              singleRun$template2Mem <- sub("path_to_globalTemplate",
