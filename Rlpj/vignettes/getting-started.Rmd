...
The **runLPJ** function reads the setup parallel object and creates a cluster to which submits the model wrapper function with its respective parameters. To understand how to use both setupParallel and runLPJ, you might want first to see what happens in a single model run.
...
Before calling the *setupParallel* function, we will prepare the input parameters for the function.  In this case, we want to test 20 different values for the *emax* parameter. Concerning the outputs, we are only interested in *aaet* and *lai* outputs.
...
Now that we have processed the output data, we can plot the data and see how the variables evolve across the years. The *plotData* function will plot the data as a time series using the zoo library. 
